...
class MagNet3Frames(object):
    ...
    def _encoder(self, image):
        enc = res_encoder(
            image, layer_dims=self.encoder_dims, num_resblk=self.num_enc_resblk
        )
        ...
        if self.use_texture_conv:
            ...
            texture_enc = tf.nn.relu(
                conv2d(
                    texture_enc, self.texture_dims, 3, stride, name="enc_texture_conv"
                )
            )
        else:
            assert self.texture_dims == self.encoder_dims, (
                "Texture dim ({}) must match encoder dim ({}) "
                "if texture_conv is not used.".format(
                    self.texture_dims, self.encoder_dims
                )
            )
            assert (
                not self.texture_downsample
            ), "Must use texture_conv if texture_downsample."
        if self.use_shape_conv:
            shape_enc = tf.nn.relu(
                conv2d(shape_enc, self.shape_dims, 3, 1, name="enc_shape_conv")
            )
        else:
    ...
    def _decoder(self, texture_enc, shape_enc):
        if self.texture_downsample:
            texture_enc = tf.compat.v1.image.resize_nearest_neighbor(
                texture_enc, tf.shape(texture_enc)[1:3] * 2
            )
            texture_enc = tf.pad(
                texture_enc, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT"
            )
            texture_enc = tf.nn.relu(
                conv2d(
                    texture_enc,
    ...
    def image_transformer(
        ...
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            with tf.compat.v1.variable_scope("manipulator"):
                self.out_shape_enc = self.manipulator(
                    self.shape_a, self.shape_b, amplification_factor
                )
    ...
    def _build_feed_model(self):
        self.test_input = tf.compat.v1.placeholder(
            tf.float32,
            [None, None, None, self.n_channels * 3],
            name="test_AB_and_output",
        )
        self.test_amplification_factor = tf.compat.v1.placeholder(
            tf.float32, [None], name="amplification_factor"
        )
        self.test_image_a = self.test_input[:, :, :, : self.n_channels]
        self.test_image_b = self.test_input[
            :, :, :, self.n_channels : (2 * self.n_channels)
        ]
        self.test_amplified_frame = self.test_input[
            :, :, :, (2 * self.n_channels) : (3 * self.n_channels)
        ]
    ...
    def inference(self, frameA, frameB, amplification_factor):
        ...
        out_amp = self.sess.run(
            self.test_output,
            feed_dict={
                self.test_input: in_frames,
                self.test_amplification_factor: [amplification_factor],
            },
        )
        return out_amp
    ...
    def _build_IIR_filtering_graphs(self):
        ...
        self.input_image = tf.compat.v1.placeholder(
            tf.float32,
            [1, self.image_height, self.image_width, self.n_channels],
            name="input_image",
        )
        self.filtered_enc = tf.compat.v1.placeholder(
            tf.float32, [1, None, None, self.shape_dims], name="filtered_enc"
        )
        self.out_texture_enc = tf.compat.v1.placeholder(
            tf.float32, [1, None, None, self.texture_dims], name="out_texture_enc"
        )
        self.ref_shape_enc = tf.compat.v1.placeholder(
            tf.float32, [1, None, None, self.shape_dims], name="ref_shape_enc"
        )
        self.amplification_factor = tf.compat.v1.placeholder(
            tf.float32, [None], name="amplification_factor"
        )
        with tf.compat.v1.variable_scope("ynet_3frames"):
            ...
            with tf.compat.v1.variable_scope("manipulator"):
                ...
                self.out_shape_enc = self.manipulator(
                    0.0, self.filtered_enc, self.amplification_factor
                )
                self.out_shape_enc += self.ref_shape_enc - self.filtered_enc
            with tf.compat.v1.variable_scope("decoder"):
                self.output_image = tf.clip_by_value(
                    self._decoder(self.out_texture_enc, self.out_shape_enc), -1.0, 1.0
                )
    ...
    def run_temporal(
        ...
        if filter_type == "fir":
            ...
            filter_b, filter_a = butter(
                n_filter_tap, [fl / nyq, fh / nyq], btype="bandpass"
            )
            filter_a = filter_a[1:]
            ...
            raise ValueError(
                "Filter type must be either "
                '["fir", "butter", "differenceOfIIR"] got ' + filter_type
            )
        head, tail = os.path.split(out_dir)
        tail = tail + "_fl{}_fh{}_fs{}_n{}_{}".format(
            fl, fh, fs, n_filter_tap, filter_type
        )
        ...
        if len(filter_a) != 0:
            ...
            for frame in tqdm(vid_frames, desc="Applying IIR"):
                ...
                out_amp = self.sess.run(
                    self.output_image,
                    feed_dict={
                        self.out_texture_enc: texture_enc,
                        self.filtered_enc: y,
    ...
    def _build_training_graph(self, train_config):
        ...
        self.reg_loss = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES
        )
        if self.reg_loss and train_config["weight_decay"] > 0.0:
            print("Adding Regularization Weights.")
            self.loss = self.loss_function(self.output, frameAmp) + train_config[
                "weight_decay"
            ] * tf.add_n(self.reg_loss)
        else:
        ...
        with tf.compat.v1.variable_scope("ynet_3frames/encoder", reuse=True):
            ...
            self.loss = (
                self.loss
                + train_config["texture_loss_weight"]
                * L1_loss(texture_c, self.texture_a)
        ...
        self.image_sum = tf.summary.image(
            "train_B_OUT", tf.concat([frameB, self.output], axis=2), max_outputs=2
        )
        if self.n_channels == 3:
            self.image_comp_sum = tf.summary.image(
                "train_GT_OUT", frameAmp - self.output, max_outputs=2
            )
            self.image_orig_comp_sum = tf.summary.image(
                "train_ORIG_OUT", frameA - self.output, max_outputs=2
            )
        else:
            self.image_comp_sum = tf.summary.image(
                "train_GT_OUT",
    ...
    def train(self, train_config):
        ...
        self.optim_op = tf.compat.v1.train.AdamOptimizer(
            self.lr, beta1=train_config["beta1"]
        ).minimize(
            self.loss,
        ...
        self.writer = tf.compat.v1.summary.FileWriter(
            train_config["logs_dir"], self.sess.graph
        )
        ...
        if train_config["continue_train"] and self.load(train_config["checkpoint_dir"]):
            print("[*] Load Success")
        elif train_config["restore_dir"] and self.load(
            train_config["restore_dir"],
        ...
        try:
            while not coord.should_stop():
                ...
                if global_step % 100 == 0:
                    # Write image summary.
                    img_sum_str, img_comp_str, img_orig_str = self.sess.run(
                        [self.image_sum, self.image_comp_sum, self.image_orig_comp_sum]
                    )
                ...
                print(
                    "Steps: %2d time: %4.4f (%4.4f steps/sec)"
                    % (global_step, elapsed_time, float(global_step) / elapsed_time)
                )
